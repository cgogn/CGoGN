...
#define CGoGN_CONTAINER_DLL_EXPORT 1
...
//	}
	...
	for (int i = int(m_holesBlocks.size() - 1); i > int(nbb); --i)
	...
	for(unsigned int j = 0; j < m_tableAttribs.size(); ++j)
	{
		if (m_tableAttribs[j] != NULL)
			m_tableAttribs[j]->setNbBlocks(uint32(m_holesBlocks.size()));
	}
}
 ...
 unsigned int AttributeContainer::insertLine()
{
	...
	{
		HoleBlockRef* ptr = new HoleBlockRef();					// new block
		unsigned int numBlock = uint32(m_holesBlocks.size());
...
//	}
	...
	bufferui.push_back(uint32(m_holesBlocks.size()));
	bufferui.push_back(uint32(m_tableBlocksWithFree.size()));
	bufferui.push_back(uint32(bufferamv.size()));
...
// 	clear is done from the map
	...
	sz = uint32(cont.m_tableAttribs.size());
	for (unsigned int i = 0; i < sz; ++i)
	{
	...
	for (unsigned int i = 0; i < sz; ++i)
	{
		...
		ptr->setIndex(uint32(m_tableMarkerAttribs.size()));
...
{
	CGoGNout << "Name ; ;";
	for (size_t i = 0; i < m_tableAttribs.size(); ++i)
	{
		if (m_tableAttribs[i] != NULL)
		{
	...
	for (size_t i = 0; i < m_tableMarkerAttribs.size(); ++i)
	{
 ...
 AttributeMultiVector<MarkerBool>* AttributeContainer::addMarkerAttribute(const std::string& attribName)
{
	...
	index = uint32(m_tableMarkerAttribs.size()) ;
...
//{
//
//	CGoGN_CONTAINER_API AttributeContainer::AttributeContainer() :
//		m_currentBrowser(NULL),
//		m_orbit(0),
//		m_nbAttributes(0),
//		m_nbUnknown(0),
...
//	**************************************/
//
//	CGoGN_CONTAINER_API unsigned int AttributeContainer::getAttributeIndex(const std::string& attribName) const
//	{
//		unsigned int index;
//		bool found = false;
//		for (index = 0; index < m_tableAttribs.size() && !found; ++index)
...
//	}
//
//	CGoGN_CONTAINER_API const std::string& AttributeContainer::getAttributeName(unsigned int index) const
//	{
//		assert(index < m_tableAttribs.size() || !"getAttributeName: attribute index out of bounds");
//		assert(m_tableAttribs[index] != NULL || !"getAttributeName: attribute does not exist");
//
...
//
//
//	CGoGN_CONTAINER_API unsigned int AttributeContainer::getAttributesNames(std::vector<std::string>& names) const
//	{
//		names.clear();
//		names.reserve(m_nbAttributes);
//
...
//
//
//	CGoGN_CONTAINER_API unsigned int AttributeContainer::getAttributesTypes(std::vector<std::string>& types)
//	{
//		types.clear();
//		types.reserve(m_nbAttributes);
//
...
//	**************************************/
//
//	CGoGN_CONTAINER_API void AttributeContainer::swap(AttributeContainer& cont)
//	{
//		// swap everything but the orbit
//
//		m_tableAttribs.swap(cont.m_tableAttribs);
...
//	}
//
//	CGoGN_CONTAINER_API void AttributeContainer::clear(bool removeAttrib)
//	{
//		m_size = 0;
//		m_maxSize = 0;
//
...
//	}
//
//	CGoGN_CONTAINER_API bool AttributeContainer::hasMarkerAttribute() const
//	{
//		// not only size() != 0 because of BoundaryMarkers !
//		for (auto it = m_tableMarkerAttribs.begin(); it != m_tableMarkerAttribs.end(); ++it)
//		{
...
//
//
//	CGoGN_CONTAINER_API void AttributeContainer::compact(std::vector<unsigned int>& mapOldNew)
//	{
//		mapOldNew.clear();
//		mapOldNew.resize(realEnd(), 0xffffffff);
//
...
//
//
//	CGoGN_CONTAINER_API unsigned int AttributeContainer::insertLine()
//	{
//		// if no more rooms
//		if (m_tableBlocksWithFree.empty())
//		{
...
//
//
//	CGoGN_CONTAINER_API void AttributeContainer::removeLine(unsigned int index)
//	{
//		unsigned int bi = index / _BLOCKSIZE_;
//		unsigned int j = index % _BLOCKSIZE_;
//
...
//
//
//	CGoGN_CONTAINER_API void AttributeContainer::saveBin(CGoGNostream& fs, unsigned int id) const
//	{
//		std::vector<AttributeMultiVectorGen*> bufferamv;
//		bufferamv.reserve(m_tableAttribs.size());
//
...
//	}
//
//	CGoGN_CONTAINER_API unsigned int AttributeContainer::loadBinId(CGoGNistream& fs)
//	{
//		unsigned int id;
//		fs.read(reinterpret_cast<char*>(&id), sizeof(unsigned int));
//		return id;
//	}
//
//	CGoGN_CONTAINER_API bool AttributeContainer::loadBin(CGoGNistream& fs)
//	{
//		if (m_attributes_registry_map == NULL)
//		{
//			CGoGNerr << "Attribute Registry non initialized" << CGoGNendl;
...
//	}
//
//	CGoGN_CONTAINER_API void  AttributeContainer::copyFrom(const AttributeContainer& cont)
//	{
//		// 	clear is done from the map
//
//		m_size = cont.m_size;
...
//	}
//
//	CGoGN_CONTAINER_API void AttributeContainer::dumpCSV() const
//	{
//		CGoGNout << "Name ; ;";
//		for (size_t i = 0; i < m_tableAttribs.size(); ++i)
//		{
...
//	}
//
//	CGoGN_CONTAINER_API void AttributeContainer::dumpByLines() const
//	{
//		CGoGNout << "Container of " << orbitName(this->getOrbit()) << CGoGNendl;
//		for (size_t i = 0; i < m_tableAttribs.size(); ++i)
//		{
...
//
//
//	CGoGN_CONTAINER_API AttributeMultiVectorGen* AttributeContainer::addAttribute(const std::string& typeName, const std::string& attribName)
//	{
//		// first check if attribute already exist
//		unsigned int index = UNKNOWN;
//		if (attribName != "")
...
//
//
//	CGoGN_CONTAINER_API AttributeMultiVector<MarkerBool>* AttributeContainer::addMarkerAttribute(const std::string& attribName)
//	{
//		// first check if attribute already exist
//		unsigned int index;
//		if (attribName != "")
...
//
//
//	CGoGN_CONTAINER_API bool AttributeContainer::removeMarkerAttribute(const std::string& attribName)
//	{
//		unsigned int index;
//		bool found = false;
//		for (index = 0; index < m_tableAttribs.size() && !found; ++index)
