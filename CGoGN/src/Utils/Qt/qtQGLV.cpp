...
void SimpleQGLV::setDock(QDockWidget *dock)
{
	m_dock = dock;
...
QDockWidget* SimpleQGLV::dockWidget()
{
	return m_dock;
}

void SimpleQGLV::setCallBack( const QObject* sender, const char* signal, const char* method)
{
	connect(sender, signal, this, method);
}

void SimpleQGLV::windowTitle(const char* windowTitle)
{
	setWindowTitle(tr(windowTitle));
}

void SimpleQGLV::dockTitle(const char* dockTitle)
{
	if (m_dock)
		m_dock->setWindowTitle(tr(dockTitle));
}

void SimpleQGLV::statusMsg(const char* msg, int timeoutms)
{
	if (msg)
	{
...
QDockWidget* SimpleQGLV::addEmptyDock()
{
...
void SimpleQGLV::visibilityDock(bool visible)
{
	if (visible)
		m_dock->show();
	else
...
void SimpleQGLV::visibilityConsole(bool visible)
{
	if (visible)
		m_dockConsole->show();
	else
...
void SimpleQGLV::toggleVisibilityDock()
{
	if (m_dock->isHidden())
		m_dock->show();
	else
...
void SimpleQGLV::toggleVisibilityConsole()
{
...
void SimpleQGLV::add_menu_entry(const std::string label, const char* method)
{
...
void SimpleQGLV::init_app_menu()
{
	m_appMenu->clear();
}

void SimpleQGLV::setHelpMsg(const std::string& msg)
{
	m_helpString = msg;
}

void SimpleQGLV::setGLWidgetMouseTracking(bool b)
{
	m_qglWidget->setMouseTracking(b);
}

void SimpleQGLV::closeEvent(QCloseEvent *event)
{
	QWidget::closeEvent(event) ;
	cb_exit();
}

void SimpleQGLV::keyPressEvent(QKeyEvent *e)
{
...
{
	QPoint xy = m_qglWidget->mapFromGlobal(QCursor::pos());
	x = xy.x();
	y = m_qglWidget->getHeight() - xy.y();
}
...
/*
void SimpleQGLV::synchronize(SimpleQT* sqt)
{
...
void SimpleQGLV::registerShader(GLSLShader* ptr)
{
	GLSLShader::registerShader(this, ptr) ;
}

void SimpleQGLV::unregisterShader(GLSLShader* ptr)
{
	GLSLShader::unregisterShader(this, ptr) ;
}

void SimpleQGLV::cb_updateMatrix()
{
...
{
	m_qglWidget->updateGL();
}

void SimpleQGLV::updateGLMatrices()
{
}
...
void SimpleQGLV::transfoScale(float sx, float sy, float sz)
{
	transfoMatrix() = glm::scale(transfoMatrix(), glm::vec3(sx,sy,sz));
}

void SimpleQGLV::pushTransfoMatrix()
{
	m_stack_trf.push(transfoMatrix());
}

bool SimpleQGLV::popTransfoMatrix()
{
...
std::string SimpleQGLV::selectFile(const std::string& title, const std::string& dir, const std::string& filters)
{
    QString fileName = QFileDialog::getOpenFileName(this, tr(title.c_str()), tr(dir.c_str()), tr(filters.c_str()), 0, 0);
    return fileName.toStdString();
}

std::string SimpleQGLV::selectFileSave(const std::string& title, const std::string& dir, const std::string& filters)
{
    QString fileName = QFileDialog::getSaveFileName(this, tr(title.c_str()), tr(dir.c_str()), tr(filters.c_str()), 0, 0);
    return fileName.toStdString();
}

void SimpleQGLV::cb_about_cgogn()
{
...
void SimpleQGLV::cb_about()
{
   QMessageBox::about(this, tr("About App"), m_helpString.c_str());
}

void SimpleQGLV::snapshot(const QString& filename, const char* format, const int& quality)
{
	QImage im = m_qglWidget->grabFrameBuffer(false);
	im.save(filename, format, quality);
}

void SimpleQGLV::setGeometry(int x, int y, int w, int h)
{
	move(x,y);
	resize(w,h);
}
