...
//
//		VEC3 pos;
//		pos[0] = properties[0]; pos[1] = properties[1]; pos[2] = properties[2];
//
//		unsigned int id = m_container.insertLine();
//		m_positions[id] = pos;
//		verticesID.push_back(id);
...
template <typename PFP>
bool MeshTablesSurface<PFP>::importCTM(const std::string& filename)
{
...
template<typename PFP>
void MeshTablesSurface<PFP>::extractMeshRec(const struct aiScene* scene, const struct aiNode* nd, struct aiMatrix4x4* trafo)
{
...
//		std::cout << "Mesh num "<<n<< std::endl;
		...
		for (unsigned int t = 0; t < mesh->mNumVertices; ++t)
		{
			...
			unsigned int id = m_container.insertLine();
			m_positions[id] = VEC3(tmp[0],tmp[1],tmp[2]);
//			(*m_labels)[id] = m_lab++;
			verticesID.push_back(id);
		}
...
//		std::cout << "Children "<<n<< std::endl;
		extractMeshRec(scene, nd->mChildren[n],trafo);
	}
	*trafo = prev;
}

template <typename PFP>
bool MeshTablesSurface<PFP>::importASSIMP(const std::string& filename)
{
	m_nbVertices = 0;
	m_nbFaces = 0;
	...
	extractMeshRec(scene, scene->mRootNode, &trafo);

	return true;
}

template <typename PFP>
bool MeshTablesSurface<PFP>::importPlyPTM(const std::string& filename, typename PFP::TFRAME& Frame, typename PFP::TRGBFUNCS& RGBfunctions)
{
	std::ifstream fp(filename.c_str(), std::ios::binary);
	if (!fp.good())
	{
	...
	for (unsigned int i = 0; i < nbp; ++i)
	{
		...
		m_positions[id] = VEC3(properties[0],properties[1],properties[2]);
