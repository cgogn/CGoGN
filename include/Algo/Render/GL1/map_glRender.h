...
void renderTriQuadPoly(typename PFP::MAP& the_map, RenderType rt, float explode,
		const AttributeHandler<typename PFP::VEC3, VERTEX>& position, const AttributeHandler<typename PFP::VEC3, VERTEX>& normal, const FunctorSelect& good = allDarts);

template <typename PFP>
void renderTriQuadPoly(typename PFP::MAP& the_map, RenderType rt, float explode,
		const AttributeHandler<typename PFP::VEC3, VERTEX>& position, const AttributeHandler<typename PFP::VEC3, VERTEX>& normal, const AttributeHandler<typename PFP::VEC3, VERTEX>& color, const FunctorSelect& good = allDarts);

template <typename PFP>
void renderNormalVertices(typename PFP::MAP& the_map,
		const AttributeHandler<typename PFP::VEC3, VERTEX>& position, const AttributeHandler<typename PFP::VEC3, VERTEX>& normal, float scale, const FunctorSelect& good = allDarts);

template <typename PFP>
void renderFrameVertices(typename PFP::MAP& the_map,
		const AttributeHandler<typename PFP::VEC3, VERTEX>& position, const AttributeHandler<typename PFP::VEC3, VERTEX> frame[3], float scale, const FunctorSelect& good = allDarts);
