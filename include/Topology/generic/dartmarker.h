...
public:
	 ...
	 */
	DartMarker(GenericMap& map): m_map(map)
	{
		m_marker = map.getNewMarker(DART_ORBIT);
	}
...
public:
	...
	{
		m_map.getMarkerVector(DART_ORBIT)->operator[](d.index).setMark(m_marker);
	}
	...
	{
		m_map.getMarkerVector(DART_ORBIT)->operator[](d.index).unsetMark(m_marker);
	}
	...
	{
		return m_map.getMarkerVector(DART_ORBIT)->operator[](d.index).testMark(m_marker);
	}
	...
	{
		m_map.getMarkerVector(DART_ORBIT)->operator[](em).setMark(m_marker);
	}
	...
	{
		m_map.getMarkerVector(DART_ORBIT)->operator[](em).unsetMark(m_marker);
	}
	...
	{
		return m_map.getMarkerVector(DART_ORBIT)->operator[](em).testMark(m_marker);
	}
	...
	virtual void markAll()
	{
		...
		for (unsigned int i = cont.begin(); i != cont.end(); cont.next(i))
			m_map.getMarkerVector(DART_ORBIT)->operator[](i).setMark(m_marker);
	}
	...
	virtual void unmarkAll()
	{
		AttributeContainer& cont = m_map.getAttributeContainer(DART_ORBIT) ;
		for (unsigned int i = cont.begin(); i != cont.end(); cont.next(i))
			m_map.getMarkerVector(DART_ORBIT)->operator[](i).unsetMark(m_marker);
	}

	virtual void markOrbit(unsigned int orbit, Dart d)
	{
		FunctorMark<GenericMap> fm(m_map, m_marker, m_map.getMarkerVector(DART_ORBIT)) ;
		m_map.foreach_dart_of_orbit(orbit, d, fm) ;
	}

	virtual void unmarkOrbit(unsigned int orbit, Dart d)
	{
		FunctorUnmark<GenericMap> fm(m_map, m_marker, m_map.getMarkerVector(DART_ORBIT)) ;
		m_map.foreach_dart_of_orbit(orbit, d, fm) ;
	}
};
...
public:
	...
	void unmarkAll()
	{
		for (std::vector<unsigned int>::iterator it = m_markedDarts.begin(); it != m_markedDarts.end(); ++it)
			m_map.getMarkerVector(DART_ORBIT)->operator[](*it).unsetMark(m_marker);
	}
};
