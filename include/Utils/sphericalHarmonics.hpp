...
SphericalHarmonics<Tscalar,Tcoef>::SphericalHarmonics()
{
	...
	for (int i=0; i<nb_coefs; i++)
		coefs[i] = Tcoef (0); // Tcoef (0.0,0.0,0.0); // TODO : use Tcoef (0)
}
...
void SphericalHarmonics<Tscalar,Tcoef>::fit_to_data(int n,
													Tdirection* t_theta, Tdirection* t_phi,
													Tchannel* t_R, Tchannel* t_G, Tchannel* t_B,
													double lambda)
{
	Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> mM (nb_coefs,n); // matrix with basis function values, evaluated for all directions
	// compute mM
	for (int p=0; p<n; ++p)
	{
...
void SphericalHarmonics<Tscalar,Tcoef>::fit_to_data(int n,
													Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic>& mM,
													Tchannel* t_R, Tchannel* t_G, Tchannel* t_B,
													double lambda)
{
	...
	// allocate the memory
	Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> mA (nb_coefs,nb_coefs); // matrix A in linear system AC=B
	Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> mB (nb_coefs,3); // matrix B in linear system AC=B : contains [t_R, t_G, t_B]
	Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> mC (nb_coefs,3); // matrix C (solution) in linear system AC=B : contains the RGB coefs of the resulting SH
	...
	Eigen::LDLT< Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> > solver (mA);
	mC = solver.solve(mB);
