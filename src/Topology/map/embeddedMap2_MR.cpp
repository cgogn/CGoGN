...
Dart EmbeddedMap2_MR::newPolyLine(unsigned int nbEdges)
{
...
Dart EmbeddedMap2_MR::newFace(unsigned int nbEdges, bool withBoundary)
{
	...
	if(withBoundary)
	{
		...
		{
/*
			Traversor2FV<EmbeddedMap2_MR> t(*this, d);
...
*/
		...
		{
/*
			Traversor2FE<EmbeddedMap2_MR> t(*this, d);
			for(Dart it = t.begin(); it != t.end(); it = t.next())
...
void EmbeddedMap2_MR::splitVertex(Dart d, Dart e)
{
	Dart dd = phi2(d) ;
	Dart ee = phi2(e) ;
...
Dart EmbeddedMap2_MR::deleteVertex(Dart d)
{
...
Dart EmbeddedMap2_MR::cutEdge(Dart d)
{
...
bool EmbeddedMap2_MR::uncutEdge(Dart d)
{
	if(Map2::uncutEdge(d))
	{
		if(isOrbitEmbedded<EDGE>())
...
bool EmbeddedMap2_MR::edgeCanCollapse(Dart d)
{
...
Dart EmbeddedMap2_MR::collapseEdge(Dart d, bool delDegenerateFaces)
{
...
bool EmbeddedMap2_MR::flipEdge(Dart d)
{
	if(Map2::flipEdge(d))
	{
		Dart e = phi2(d) ;
...
bool EmbeddedMap2_MR::flipBackEdge(Dart d)
{
...
void EmbeddedMap2_MR::swapEdges(Dart d, Dart e)
{
	Dart d2 = phi2(d);
	Dart e2 = phi2(e);
	Map2::swapEdges(d,e);
...
void EmbeddedMap2_MR::insertEdgeInVertex(Dart d, Dart e)
{
...
bool EmbeddedMap2_MR::removeEdgeFromVertex(Dart d)
{
	...
	if (isOrbitEmbedded<VERTEX>())
	{
		...
		copyCellAttributes<VERTEX>(d, dPrev);
	}
	...
	if (isOrbitEmbedded<FACE>())
	{
		if(!sameFace(d, dPrev))
		{
			...
			copyCellAttributes<FACE>(d, dPrev);
		}
		else
		{
...
void EmbeddedMap2_MR::sewFaces(Dart d, Dart e, bool withBoundary)
{
	if (!withBoundary)
	{
...
void EmbeddedMap2_MR::unsewFaces(Dart d, bool withBoundary)
{
	if (!withBoundary)
	{
	...
	if (isOrbitEmbedded<VERTEX>())
	{
		...
		if(!sameVertex(d, ee))
		{
			...
			copyCellAttributes<VERTEX>(ee, d);
		}
		...
		if(!sameVertex(e, dd))
		{
			...
			copyCellAttributes<VERTEX>(dd, e);
		}
	}
...
bool EmbeddedMap2_MR::collapseDegeneratedFace(Dart d)
{
	Dart e = phi2(d) ;
...
void EmbeddedMap2_MR::splitFace(Dart d, Dart e)
{
	Map2::splitFace(d, e) ;
...
bool EmbeddedMap2_MR::mergeFaces(Dart d)
{
	Dart dNext = phi1(d) ;
...
bool EmbeddedMap2_MR::mergeVolumes(Dart d, Dart e, bool deleteFace)
{
...
void EmbeddedMap2_MR::splitSurface(std::vector<Dart>& vd, bool firstSideClosed, bool secondSideClosed)
{
	std::vector<Dart> darts ;
	...
	for(unsigned int i = 0; i < vd.size(); ++i)
	{
		...
		if(isOrbitEmbedded<EDGE>())
		{
			...
			copyCellAttributes<EDGE>(dit2, dit);
		}
...
unsigned int EmbeddedMap2_MR::closeHole(Dart d, bool forboundary)
{
